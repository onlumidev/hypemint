<template>
  <div>
  <section class="fold-press text-white border-b py-8">

    <div class="container mx-auto pt-4">

      <div class="flex flex-wrap items-center">
  <div class="w-5/6 sm:w-1/2 p-6 not-on-mobile">

  </div>
  <div class="w-full sm:w-1/2 p-6">
    <div class="container relative m-auto not-on-desktop">
    <img src="~static/images/cartas.png" class="nft-qr nft-cartas"  />
</div>


    <h3 class="press-title">
      {{ $t('ourcompanies') }} <span class="green">Gaming NFTs</span>
    </h3>


            <p class="mkt-text">
              {{ $t('game_text') }}
            </p>

            <ul class="mkt-bullets">
              <li>{{ $t('game_bullet1') }}</li>

            </ul>

            <div class="container containermin mx-auto ">
              <h3 class="press-title mt-20">
                Get in
      <span class="green">touch</span>
    </h3><a id="press" style="position: absolute;top: 0;"></a>

    <a id="gaming" style="position: absolute;bottom: 150px;"></a>



<div id="mc_embed_signup">
    <form action="https://hypemint.us9.list-manage.com/subscribe/post?u=256961a89b00c5d962724cbd0&amp;id=7aa9097a16&amp;f_id=008812e1f0" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
        <div id="mc_embed_signup_scroll">

        <div class="indicates-required"><span class="asterisk">*</span> indicates required</div>
<div class="mc-field-group">
	<label for="mce-EMAIL">Email Address  <span class="asterisk">*</span>
</label>
	<input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL" required>

</div>
<div class="mc-field-group">
	<label for="mce-MESSAGE">Message  <span class="asterisk">*</span>
</label>
	<input type="text" value="" name="MESSAGE" class="required" id="mce-MESSAGE" required>

</div>
<div class="mc-field-group">
	<label for="mce-MMERGE6">Phone </label>
	<input type="text" value="" name="MMERGE6" class="" id="mce-MMERGE6">

</div>
	<div id="mce-responses" class="clear foot">
		<div class="response" id="mce-error-response" style="display:none"></div>
		<div class="response" id="mce-success-response" style="display:none"></div>
	</div>    <!-- real people should not fill this in and expect good things - do not remove this or risk form bot signups-->
    <div style="position: absolute; left: -5000px;" aria-hidden="true"><input type="text" name="b_256961a89b00c5d962724cbd0_7aa9097a16" tabindex="-1" value=""></div>
        <div class="optionalParent">
            <div class="clear foot">
                <input type="submit" value="Send" name="subscribe" id="mc-embedded-subscribe" class="btn button">
                <p class="brandingLogo"><a href="http://eepurl.com/ian8yT" title="Mailchimp - email marketing made easy and fun"><img src="https://eep.io/mc-cdn-images/template_images/branding_logo_text_dark_dtp.svg"></a></p>
            </div>
        </div>
    </div>
</form>
</div>

    </div>



  </div>
</div>


    </div>




  </section>
  <div class="container relative m-auto not-on-mobile">
    <img src="~static/images/cartas.png" class="nft-qr nft-cartas"  />
</div>

</div>

</template>

<style>
  .fold-press .border-title {
    border-bottom: 1px solid #00cc63;
    display: inline-block;
    margin-bottom: 20px;
    font-size: 20px;
  }

  .fold-press .slick-track.slick-center{

      display: flex;

      justify-content: space-evenly;
  }

  .fold-press .slick-track.slick-center{
      display: flex;
      justify-content: space-evenly;
  }

  .fold-press .slick-dots {
      bottom: -75px;
      margin-left: 4px;
  }

  .fold-press .slick-dots li button:before {
      color: #000;
  }

  .fold-press .slick-dots li.slick-active button:before {
      opacity: 1;
      color: #000;
  }



  .anypress {
    padding-left: 11px;
    font-size: 20px;
    display: block;
    font-weight: 600;
  }

  .contact-button {
    display: block;
    background: #000;
    max-width: 321px;
    color: #fff;
    text-align: center;
    padding: 7px 0;
    margin-top: 9px;
    font-size: 17px;
    font-weight: 500;
  }

  .contact-button:hover {
    background: #00cc63;
    color: #000;
  }



  .noticias-box > img {
    height: 30px;
  }

  .noticias-box p {
    font-size: 13px;
    margin: 30px 0;
    max-width: 347px;
  }

  .noticias-box p.fs-12 {
    font-size: 12px;
  }

  .noticias-box .closequote img {
    display: inline-block;
  }

  .fold-press .slick-list{
    overflow: inherit;
  }

  .fold-press .slick-arrow {
    display: none!important;
  }

  .fold-press  .slick-slide > div {
    display: block;
    box-shadow: 12px 1px 25px rgb(0 0 0 / 10%);
    margin-right: 42px;
    padding: 42px 9px;
  }

  .pt-20 {
    padding-top: 70px;
  }

@media only screen and (max-width: 1320px) {
 .fold-press div .containermin {
    max-width: inherit;
  }

  .fold-press .slick-track.slick-center {
      display: flex;
      justify-content: normal;
  }

  .fold-press .slick-initialized .slick-slide {
      padding-right: 15px;
  }
}
 @media only screen and (max-width: 768px) {

     .xs-hidden {
      display: none;
    }

    h3 {
      font-size: 34px;
    }

    .joy-title {
        font-size: 22px;

    }

    .fold-press  .slick-slide > div {

      margin-right: 0;

    }

     .fold-press .flex .p-6 {
        width: 100%;
      }

      .anypress {
        text-align: center;
      }

      .contact-button {
          display: block;
          background: #000;
          max-width: 321px;
          color: #fff;
          text-align: center;
          padding: 7px 0;
          margin-top: 9px;
          font-size: 17px;
          font-weight: 500;
          margin: 9px auto 0;
      }

      .press-title {
          font-size: 36px;
          line-height: 45px;
          max-width: 384px;
          text-align: center;
          margin: 35px auto 20px;
      }

 }
</style>

<script>

  export default {
      name: 'Press'
  }
</script>

<i18n>
{
  "en": {
    "ourcompanies": "We do",
    "game_text": "Actually it’s very popular in the game market and is generating billions of dollars every month, quarter and year. It’s easy to integrate and with high demand at the moment, our studio is prepared for the best and most complex projects you can imagine. We can create arts and rewards in order to make your business grow exponentially, reaching large audiences to offer various monetization opportunities.",
    "game_bullet1": "Go beyond the fun of games, profit too."
  }
}
</i18n>
